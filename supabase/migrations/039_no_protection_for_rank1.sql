-- ============================================================
-- Migration 039: Remove shield (challenged_protection) for rank #1
-- The #1 ranked player should always be challengeable — no protection.
-- ============================================================

-- Update validate_challenge_creation to skip protection check for position 1
  ...
  IF v_rule_rematch THEN
    ...
    IF v_last_match_date IS NOT NULL THEN
      IF NOT EXISTS (
        ...
        RETURN jsonb_build_object('valid', FALSE, 'error',
          'O adversario precisa jogar contra outro jogador antes de uma revanche');
      END IF;
    END IF;
  END IF;
